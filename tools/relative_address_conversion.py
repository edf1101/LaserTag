...
def main() -> None:
    ...
    try:  # try to remove old backup
        ...
        pass
    shutil.copytree(path.abspath(path_to_src), backup_directory,
                    dirs_exist_ok=True)  # copy in new backup
    ...
    for file_path in all_file_paths:  # iterate through every file in the 'all_file_paths' list
        ...
        for ind, line in enumerate(file_data):
            if '#include' in line and '"' in line and 'Arduino.h' not in line:
                ...
                # Get the directory part of file we are inspecting
                abs_path_dir = path.dirname(absolute_path)
                # Get the relative bit to copy in
                relative_to_current = path.relpath(full_name, abs_path_dir)

                # splice the replacement path into the original line
                replacement = (line[:line.find('"') + 1] +
                               relative_to_current + line[line.rfind('"'):])
